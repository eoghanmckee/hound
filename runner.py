...
class Runner(object):
	...
	def __init__(self):
		self.app = create_app()
		self.bearer_token = Crowdstrikechecker().crwd_token_generator()
		...
		self.user_ro_connection = ''
		if self.app.config['CONFIG']['postgres_host']:
			try:
				self.user_ro_connection = psycopg2.connect(host = self.app.config['CONFIG']['postgres_host'],
	...
	def iocanalysis(self, interval, config_class=Config):
		...
		with self.app.app_context():
			...
			cases = Cases.query.filter_by(status=1).all()

			if not cases:
				return

			for case in cases:
				t = Thread(target=self.threader, args=(case,interval,))
				t.start()
				t.join()

			self.app.logger.info('Finished checking IOCs.')

			if self.user_ro_connection:
				self.user_ro_connection.close()

	def threader(self, case, interval):
		with self.app.app_context():
			message = ''
			...
			all_iocs = {
				...
				"userid": UserIDs.query.filter_by(caseid=case.id).all(),
			...
			if interval == 1:
				...
				if currentcase.crowdstrike == 1 and self.bearer_token:
					crwd_results = Crowdstrikechecker().ioc_checker(self.bearer_token, all_iocs, case.id)
				...
				if currentcase.postgres == 1:
					pg_results = Postgreschecker().ioc_checker(all_iocs, self.user_ro_connection, case.id)
